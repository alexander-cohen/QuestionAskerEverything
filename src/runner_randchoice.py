...
data_matrix_raw = np.loadtxt("../data/intel_dat.txt")
important_feats = pickle.load(open('../data/important_feats.pickle', 'r'))

data_matrix = data_matrix_raw[:,important_feats]
print np.shape(data_matrix)

with open("../data/intel_feat.txt", "r") as feats:
    features_raw = [l[:-2] for l in feats]

features = [features_raw[f] for f in important_feats]
...
#       if len(self.items_guessed) > 0: self.prob_knowledge_from_items[np.array(self.items_guessed)] = 0
    ...
    def expected_info_gain_old(self, feature):
        eig = 0
        for i in range(5):
    ...
    def expected_info_gain(self, feature):
        eig = 0
        for i in range(5):
            eig += self.prob_response(feature, index_to_prob(i)) * self.prob_win_if_end(self.prob_with_new_knowledge((feature, index_to_prob(i))))
    ...
    def expected_info_gains(self):
        ...
        return_arr = p.map(expected_info_gain, self.features_left)
        p.close()
        print time.time() - t
    ...
    def iterate(self):
        ...
        '''          
        info_gains = [np.max(self.prob_with_new_knowledge((best_feature, 1.0))),
                      np.max(self.prob_with_new_knowledge((best_feature, 0.5))),
                      np.max(self.prob_with_new_knowledge((best_feature, 0.0))),
                      np.max(self.prob_with_new_knowledge((best_feature, -0.5))),
                      np.max(self.prob_with_new_knowledge((best_feature, -1.0)))]
        '''       
        #print self.prob_with_new_knowledge((best_feature, -1.0))
...
def expected_info_gain(feature):
    global player
    eig = 0
    for i in range(5):
